...
**The site has a domain, and the README has pictures.** `runlogapp.com` now
resolves to the Pages project, so it replaces the `runlog-aul.pages.dev`
address in the canonical link, `robots.txt` and `sitemap.xml` — the pages.dev
host still serves the same build, it is simply no longer the one to be indexed
under.

The README was a single 187-line page that described an interactive product in
prose and showed none of it. It now opens with a centred masthead and seven
screenshots captured from the real build, and the reference material behind it
moved into `docs/`: [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) for the parsers,
the pipeline and the widget contract, [`DESIGN.md`](docs/DESIGN.md) for the
choices that had plausible alternatives, and [`PRIVACY.md`](docs/PRIVACY.md)
for the three things that talk to a server. The screenshots are generated by
`npm run screenshots`, which drives Chromium through the demo run and
photographs cards by their widget id — so a renamed widget fails the script
rather than quietly producing a picture of the wrong card.
