...
Every file in [`fixtures/`](fixtures/) is generated by [`scripts/make-fixtures.mjs`](scripts/make-fixtures.mjs), not recorded. A GPS track says where somebody actually was, and this project argues that a run belongs to the person who ran it — committing one would contradict the thing the app exists to protect. The generated FIT is a genuine FIT 2.0 file with correct CRCs, so the tests still catch the day the decoder drifts away from the format. Activity files left at the repository root are gitignored, so your own run cannot be committed by accident.
